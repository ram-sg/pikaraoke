...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        -y|--yes) CONFIRM="n" ;;
        -l|--local) LOCAL="y" ;; # this installs biaoke from local source
...
if [ -z "$DISPLAY_LIST" ]; then
    INSTALL_LIST="biaoke (via uv)"
else
    INSTALL_LIST="$DISPLAY_LIST, biaoke (via uv)"
fi
...
# Install biaoke
echo "Installing biaoke via uv..."

if uv tool list | grep -q "biaoke"; then
    echo "Biaoke is already installed. Upgrading..."
    if [ "$LOCAL" == "y" ]; then
...
if [ $INSTALL_SHORTCUTS -eq 1 ]; then
    ...
    BIAOKE_BIN=$(command -v biaoke || echo "$HOME/.local/bin/biaoke")
    SHARE_DIR="$HOME/.local/share/biaoke"
    mkdir -p "$SHARE_DIR"
    ICON_PATH="$SHARE_DIR/logo.icns"
    ICON_URL="https://raw.githubusercontent.com/vicwomg/biaoke/refs/heads/master/biaoke/static/icons/logo.icns"
    if [ ! -f "$ICON_PATH" ]; then
...
EOF
        ...
        create_macos_app "Biaoke" ""
        create_macos_app "Biaoke (headless)" "--headless"
...
EOF
        ...
        if [ -d "$HOME/Desktop" ]; then
            create_linux_desktop "Biaoke" "" "Biaoke.desktop"
            create_linux_desktop "Biaoke (headless)" "--headless" "Biaoke-headless.desktop"
