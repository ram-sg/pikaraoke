...
class TestHasJsRuntime:
    ...
    def test_has_runtime(self):
        ...
        with patch("biaoke.lib.get_platform.get_installed_js_runtime", return_value="node"):
            assert has_js_runtime() is True

    def test_no_runtime(self):
        """Test when no JS runtime is available."""
        with patch("biaoke.lib.get_platform.get_installed_js_runtime", return_value=None):
...
class TestGetPlatform:
    ...
    def test_osx_platform(self):
        ...
        with patch("sys.platform", "darwin"):
            with patch("biaoke.lib.get_platform.is_android", return_value=False):
                with patch("biaoke.lib.get_platform.is_raspberry_pi", return_value=False):
    ...
    def test_windows_platform(self):
        ...
        with patch("sys.platform", "win32"):
            with patch("biaoke.lib.get_platform.is_windows", return_value=True):
                with patch("biaoke.lib.get_platform.is_android", return_value=False):
                    with patch("biaoke.lib.get_platform.is_raspberry_pi", return_value=False):
    ...
    def test_linux_platform(self):
        ...
        with patch("sys.platform", "linux"):
            with patch("biaoke.lib.get_platform.is_windows", return_value=False):
                with patch("biaoke.lib.get_platform.is_android", return_value=False):
                    with patch("biaoke.lib.get_platform.is_raspberry_pi", return_value=False):
    ...
    def test_android_platform(self):
        ...
        with patch("sys.platform", "linux"):
            with patch("biaoke.lib.get_platform.is_android", return_value=True):
    ...
    def test_raspberry_pi_platform(self):
        ...
        with patch("sys.platform", "linux"):
            with patch("biaoke.lib.get_platform.is_android", return_value=False):
                with patch("biaoke.lib.get_platform.is_raspberry_pi", return_value=True):
        ...
        with patch("biaoke.lib.get_platform.is_raspberry_pi", return_value=False):
            with patch("biaoke.lib.get_platform.is_windows", return_value=True):
                with patch("os.path.exists", return_value=True):
                    with patch(
                        "os.path.expanduser", return_value="C:\\Users\\test\\biaoke\\songs"
                    ):
                        result = get_default_dl_dir("windows")
                        assert "biaoke\\songs" in result
    ...
    def test_windows_path(self):
        ...
        with patch("biaoke.lib.get_platform.is_windows", return_value=True):
            with patch.dict(os.environ, {"APPDATA": "C:\\Users\\Test\\AppData\\Roaming"}):
                ...
                with patch("biaoke.lib.get_platform.os.path") as mock_path:
                    ...
                    assert result == "C:\\Users\\Test\\AppData\\Roaming\\biaoke"
            ...
            with patch.dict(os.environ, {"APPDATA": "C:\\Users\\Test\\AppData\\Roaming"}):
                with patch("os.makedirs") as mock_makedirs:
                    ...
                    with patch("biaoke.lib.get_platform.os.path") as mock_path:
                        ...
                        expected_path = "C:\\Users\\Test\\AppData\\Roaming\\biaoke"
                        mock_makedirs.assert_called_once_with(expected_path)

    def test_linux_path(self):
        """Test that Linux/Mac returns the home directory path."""
        with patch("biaoke.lib.get_platform.is_windows", return_value=False):
            with patch("os.path.expanduser", return_value="/home/test/.biaoke"):
                with patch("os.path.exists", return_value=True):
                    result = get_data_directory()
                    assert result == "/home/test/.biaoke"
